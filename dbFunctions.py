...
def searchQuery(database, queryText="", lastId=0):
    ...
    """
    if not isinstance(lastId, int) or lastId < 0:
        raise ValueError('lastId must be greater then or equal to 0')
    else:
        return database.tracks.find({"_id":{"$gt":lastId},"track_name":re.Regex(".*" + str(queryText) + ".*", 'i')}).limit(30)
...
def downloadTrackImage(database, track_id, download_path):
    ...
    fullpath = ""
    if str(download_path).find("/") == -1:
        fullpath = str(download_path).replace("\\", "/")
    else:
        fullpath = download_path
    if fullpath[-1] != "/":
        fullpath += "/"
    fs = gridfs.GridFS(database)
